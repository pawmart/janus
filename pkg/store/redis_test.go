...
func (s *mockRedisStore) Set(key string, value string, expire time.Duration) error {
	conn := s.getConnection()
	defer conn.Close()
...
func TestRedisStore_getSetCommandAndArgs(t *testing.T) {
	...
	seconds := 3600
	command, args = getSetCommandAndArgs(testKey, testValue, time.Duration(seconds)*time.Second)
	assert.Equal(t, "SETEX", command)
	assert.Len(t, args, 3)
	assert.Equal(t, testKey, args[0])
	assert.Equal(t, float64(seconds), args[1])
...
func TestRedisStore_Set(t *testing.T) {
	...
	argsSetEx = append(argsSetEx, float64(seconds))
	...
	assert.Nil(t, instanceSetEx.Set(testKey, testValue, time.Duration(seconds)*time.Second))
}
